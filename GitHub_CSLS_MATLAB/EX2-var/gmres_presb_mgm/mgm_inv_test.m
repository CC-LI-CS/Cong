...
T=1; tol=1e-3; mu1=1;mu2=0;  LEVEL0=3; %3
...
%                 for j=1:M
%                     for i=1:M+1
%                         A1(j,i)=(20+xpgrid(i)^2).*(20+ygrid(j)^2);
%                     end
%                 end
% 
% 
...
%                 for j=1:M+1
%                     for i=1:M
%                         A2(j,i)=(20+xgrid(i)^2).*(20+ypgrid(j)^2);
%                     end
%                 end
%                 A=1/(h^2)*(kron(Dx.',Iy)*diag(A1(:))*kron(Dx,Iy)+kron(Ix,Dy.')*diag(A2(:))*kron(Ix,Dy))+(lamj)*speye(M^2);
Mx=M;
My=M;
